const STATE_MAP = {
  ...
  DC: 'District of Columbia', FM: 'Micronesia',    FL: 'Florida',
...
/**
 * Converts a comma-separated string of state abbreviations to full names.
 * "CA,NV,UT" → "California · Nevada · Utah"
 */
export const expandStates = (statesStr, separator = ' · ') => {
  if (!statesStr) return '';
  return statesStr
    .split(',')
    .map((code) => STATE_MAP[code.trim()] ?? code.trim())
    .join(separator);
};
